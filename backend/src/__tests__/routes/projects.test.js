...
describe('Project Routes', () => {
  ...
  describe('GET /api/projects', () => {
    ...
    test('should filter by client and status', async () => {
      mockDb.all.mockImplementation((query, params, callback) => callback(null, []));

      const response = await request(app).get('/api/projects?clientId=2&status=completed');

      expect(response.status).toBe(200);
      expect(mockDb.all).toHaveBeenCalledWith(
        expect.stringContaining('AND p.status = ?'),
        ['test@example.com', 2, 'completed'],
        expect.any(Function)
      );
    });
  ...
  describe('POST /api/projects', () => {
    test('should create a project', async () => {
      ...
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Website Redesign', clientId: 2, startDate: '2024-03-01', status: 'active' });
      ...
      expect(mockDb.run).toHaveBeenCalledWith(
        ...
        ['Website Redesign', null, 2, '2024-03-01', 'active', 'test@example.com'],
        expect.any(Function)
      );
    });
    ...
    test('should default status to active', async () => {
      ...
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Website Redesign', clientId: 2 });

      expect(response.status).toBe(201);
      expect(mockDb.run).toHaveBeenCalledWith(
        expect.any(String),
        ['Website Redesign', null, 2, null, 'active', 'test@example.com'],
        expect.any(Function)
      );
    });
    ...
    test('should return 400 for missing name', async () => {
      const response = await request(app).post('/api/projects').send({ clientId: 2 });
    ...
    test('should return 400 for invalid status', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'Website Redesign', clientId: 2, status: 'archived' });
    ...
    test('should handle insert error', async () => {
      mockDb.get.mockImplementation((query, params, callback) => callback(null, { id: 2 }));
      mockDb.run.mockImplementation(function(query, params, callback) {
  ...
  describe('PUT /api/projects/:id', () => {
    ...
    test('should verify new client ownership when reassigning', async () => {
      ...
      const response = await request(app).put('/api/projects/1').send({ clientId: 5 });
