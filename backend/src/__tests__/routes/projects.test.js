...
describe('Project Routes', () => {
  ...
  describe('GET /api/projects', () => {
    test('should return all projects for authenticated user', async () => {
      const mockProjects = [mockProjectRow, { ...mockProjectRow, id: 2, name: 'Mobile App' }];
  ...
  describe('GET /api/projects/:id', () => {
    test('should return specific project', async () => {
      mockDb.get.mockImplementation((query, params, callback) => {
        callback(null, mockProjectRow);
      });

      const response = await request(app).get('/api/projects/1');

      expect(response.status).toBe(200);
      expect(response.body).toEqual({ project: mockProjectRow });
    });
  ...
  describe('POST /api/projects', () => {
    test('should create new project with valid data', async () => {
      ...
      expect(response.body.project).toEqual(mockProjectRow);
    });
    ...
    test('should default status to active when not provided', async () => {
      ...
      const response = await request(app)
        .post('/api/projects')
        .send({ name: 'No Status Project', clientId: 1, startDate: '2024-01-15' });
    ...
    test('should return 400 for invalid status', async () => {
      const response = await request(app)
        .post('/api/projects')
        .send({ ...validProject, status: 'cancelled' });
  ...
  describe('PUT /api/projects/:id', () => {
    test('should update project name', async () => {
      const updatedProject = { ...mockProjectRow, name: 'Updated Name' };

      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, mockProjectRow); // Project exists
      });
    ...
    test('should update project status', async () => {
      const updatedProject = { ...mockProjectRow, status: 'completed' };
    ...
    test('should verify client ownership when updating clientId', async () => {
      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, mockProjectRow); // Project exists
      });

      mockDb.get.mockImplementationOnce((query, params, callback) => {
        callback(null, null); // New client not found
      });
