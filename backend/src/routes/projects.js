...
router.get('/', (req, res) => {
  ...
  db.all(
    `SELECT p.id, p.name, p.description, p.client_id, p.status, p.start_date,
...
router.post('/', (req, res, next) => {
  try {
    ...
    const { name, description, clientId, status, startDate } = value;
    const db = getDatabase();

    const insertProject = () => {
      db.run(
        'INSERT INTO projects (name, description, client_id, status, start_date, user_email) VALUES (?, ?, ?, ?, ?, ?)',
        [name, description || null, clientId || null, status || 'active', startDate || null, req.userEmail],
        function(err) {
          if (err) {
            console.error('Database error:', err);
