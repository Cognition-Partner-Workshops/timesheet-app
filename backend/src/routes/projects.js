...
router.post('/', (req, res, next) => {
  try {
    ...
    const insertProject = () => {
      const dateValue = startDate ? new Date(startDate).toISOString().split('T')[0] : null;

      db.run(
        'INSERT INTO projects (name, description, client_id, start_date, status, user_email) VALUES (?, ?, ?, ?, ?, ?)',
        [name, description || null, clientId || null, dateValue, status || 'active', req.userEmail],
        function(err) {
          if (err) {
            console.error('Database error:', err);
      ...
      (err, row) => {
        ...
        function performUpdate() {
          ...
          if (value.startDate !== undefined) {
            ...
            values.push(new Date(value.startDate).toISOString().split('T')[0]);
          }
