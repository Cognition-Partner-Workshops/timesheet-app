...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    const row = await dbGet(`${PROJECT_SELECT} WHERE p.id = ?`, [result.lastID]);
...
router.put('/:id', async (req, res, next) => {
  ...
  try {
    ...
    const fieldMap = {};
    if (value.name !== undefined) fieldMap.name = value.name;
    if (value.description !== undefined) fieldMap.description = value.description;
    if (value.clientId !== undefined) fieldMap.client_id = value.clientId;
    if (value.startDate !== undefined) fieldMap.start_date = value.startDate;
    if (value.status !== undefined) fieldMap.status = value.status;
