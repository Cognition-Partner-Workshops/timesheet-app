...
router.get('/', (req, res) => {
  ...
  db.all(
    'SELECT p.*, c.name as client_name FROM projects p LEFT JOIN clients c ON p.client_id = c.id WHERE p.user_email = ? ORDER BY p.created_at DESC',
    [req.userEmail],
    (err, rows) => {
      if (err) {
...
router.get('/:id', (req, res) => {
  ...
  db.get(
    'SELECT p.*, c.name as client_name FROM projects p LEFT JOIN clients c ON p.client_id = c.id WHERE p.id = ? AND p.user_email = ?',
