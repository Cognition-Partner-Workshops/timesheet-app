...
router.post('/', (req, res, next) => {
  try {
    ...
    const { name, description, status, clientId } = value;
    const db = getDatabase();
    ...
    function insertProject() {
      db.run(
        'INSERT INTO projects (name, description, status, client_id, user_email) VALUES (?, ?, ?, ?, ?)',
        [name, description || null, status || 'active', clientId || null, req.userEmail],
        function(err) {
          if (err) {
            console.error('Database error:', err);
