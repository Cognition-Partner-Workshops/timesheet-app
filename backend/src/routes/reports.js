...
const { parseId, sendDbError } = require('../utils/routeHelpers');
...
// Load a client owned by the user along with its work entries (newest first).
// Handles validation/ownership/database errors by responding directly; only
// invokes `onSuccess(client, workEntries)` when the data is available.
function loadClientReport(req, res, onSuccess) {
...
// Sum of hours across the given work entries.
function sumHours(workEntries) {
  return workEntries.reduce((sum, entry) => sum + parseFloat(entry.hours), 0);
}

// Build a filesystem-safe report filename for a client and extension.
function reportFilename(clientName, extension) {
...
router.get('/client/:clientId', (req, res) => {
  loadClientReport(req, res, (client, workEntries) => {
    res.json({
      client: client,
...
router.get('/export/csv/:clientId', (req, res) => {
  loadClientReport(req, res, (client, workEntries) => {
    ...
    const filename = reportFilename(client.name, 'csv');
...
router.get('/export/pdf/:clientId', (req, res) => {
  loadClientReport(req, res, (client, workEntries) => {
    ...
    const filename = reportFilename(client.name, 'pdf');
