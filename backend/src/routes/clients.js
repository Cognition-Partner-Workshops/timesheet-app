...
router.get('/', (req, res) => {
  ...
  db.all(
    'SELECT id, name, description, department, email, hourly_rate, created_at, updated_at FROM clients WHERE user_email = ? ORDER BY name',
    [req.userEmail],
    (err, rows) => {
      if (err) {
...
router.get('/:id', (req, res) => {
  ...
  db.get(
    'SELECT id, name, description, department, email, hourly_rate, created_at, updated_at FROM clients WHERE id = ? AND user_email = ?',
...
router.post('/', (req, res, next) => {
  try {
    ...
    db.run(
      ...
      [name, description || null, department || null, email || null, hourlyRate !== undefined ? hourlyRate : null, req.userEmail],
      function(err) {
        if (err) {
          console.error('Database error:', err);
        ...
        db.get(
          'SELECT id, name, description, department, email, hourly_rate, created_at, updated_at FROM clients WHERE id = ?',
          [this.lastID],
          (err, row) => {
            if (err) {
              console.error('Database error:', err);
