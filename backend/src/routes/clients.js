...
router.get('/', (req, res) => {
  ...
  db.all(
    'SELECT id, name, description, department, email, billing_rate AS billingRate, created_at, updated_at FROM clients WHERE user_email = ? ORDER BY name',
    [req.userEmail],
    (err, rows) => {
      if (err) {
...
router.get('/:id', (req, res) => {
  ...
  db.get(
    'SELECT id, name, description, department, email, billing_rate AS billingRate, created_at, updated_at FROM clients WHERE id = ? AND user_email = ?',
...
router.post('/', (req, res, next) => {
  try {
    ...
    const { name, description, department, email, billingRate } = value;
    const db = getDatabase();

    db.run(
      'INSERT INTO clients (name, description, department, email, billing_rate, user_email) VALUES (?, ?, ?, ?, ?, ?)',
      [name, description || null, department || null, email || null, billingRate, req.userEmail],
      function(err) {
        if (err) {
          console.error('Database error:', err);
        ...
        db.get(
          'SELECT id, name, description, department, email, billing_rate AS billingRate, created_at, updated_at FROM clients WHERE id = ?',
          [this.lastID],
          (err, row) => {
            if (err) {
              console.error('Database error:', err);
  ...
  try {
    ...
    db.get(
      ...
      (err, row) => {
        ...
        if (value.billingRate !== undefined) {
          updates.push('billing_rate = ?');
          values.push(value.billingRate);
        }
