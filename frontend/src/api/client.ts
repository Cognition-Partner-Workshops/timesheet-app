...
class ApiClient {
  ...
  async createProject(projectData: { name: string; description?: string; clientId?: number | null; startDate?: string; endDate?: string; status?: string }) {
    const response = await this.client.post('/api/projects', projectData);
    return response.data;
  }

  async updateProject(id: number, projectData: { name?: string; description?: string; clientId?: number | null; startDate?: string; endDate?: string; status?: string }) {
