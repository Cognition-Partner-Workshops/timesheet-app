...
interface ClientResponse extends Partial<MessageResponse> {
  client: Client;
}
...
interface WorkEntryResponse extends Partial<MessageResponse> {
  workEntry: WorkEntryWithClient;
}

interface DeleteAllClientsResponse extends MessageResponse {
  deletedCount: number;
}
...
class ApiClient {
  ...
  async deleteAllClients(): Promise<DeleteAllClientsResponse> {
    const response = await this.client.delete<DeleteAllClientsResponse>('/api/clients');
    return response.data;
  }
