...
class ApiClient {
  ...
  async createProject(projectData: { name: string; description?: string; status?: string; clientId?: number | null }) {
    const response = await this.client.post('/api/projects', projectData);
    return response.data;
  }

  async updateProject(id: number, projectData: { name?: string; description?: string; status?: string; clientId?: number | null }) {
