...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error.response?.data?.error || `${isRegisterMode ? 'Registration' : 'Login'} failed. Please try again.`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <Paper elevation={3} sx={{ padding: 3, width: '100%', maxWidth: 500 }}>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <TextField
            ...
            disabled={isLoading}
            helperText={isRegisterMode ? 'Must be at least 8 characters' : undefined}
          />
          <Button
            type="submit"
            fullWidth
