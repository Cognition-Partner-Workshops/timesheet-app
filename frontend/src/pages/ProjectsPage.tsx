...
const ProjectsPage: React.FC = () => {
  ...
  const handleOpen = (project?: Project) => {
    if (project) {
      ...
      setFormData({
        ...
        budgetHours: project.budget_hours ? project.budget_hours.toString() : '',
      });
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.startDate) {
      projectData.startDate = formData.startDate.toISOString().split('T')[0];
    }
    if (formData.endDate) {
      projectData.endDate = formData.endDate.toISOString().split('T')[0];
    }
    if (formData.budgetHours) {
      projectData.budgetHours = parseFloat(formData.budgetHours);
    }
                      ...
                      <TableRow key={project.id}>
                        ...
                        <TableCell>
                          {project.budget_hours ? (
                            <Chip
                              label={`${project.budget_hours} hrs`}
                              color="primary"
                              variant="outlined"
                              size="small"
                            />
